...
if __name__ == "__main__":
    ...
    print("The dataset is divide with: \n - {} training samples \n - {} validation samples \n - {} testing samples".format(len(trainset), len(valset),
                                                                                                                           len(testset)))
    print("Creating Tensorflow dataset")

    dataset_train = tf.data.Dataset.from_tensor_slices(format_dataset(trainset)).shuffle(buffer_size=100).batch(BATCH_SIZE)
    dataset_validation = tf.data.Dataset.from_tensor_slices(format_dataset(valset)).shuffle(buffer_size=100).batch(BATCH_SIZE)
